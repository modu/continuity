...
public class ArduinoCommunicatorService extends Service {
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		if (!intent.getBooleanExtra(UsbManager.EXTRA_PERMISSION_GRANTED, false)) {
			...
			Toast.makeText(getBaseContext(),
					getString(R.string.permission_denied), Toast.LENGTH_LONG)
					.show();
			stopSelf();
			return Service.START_REDELIVER_INTENT;
		}
		...
		if (DEBUG)
			Log.i(TAG, "Receiving!");
		Toast.makeText(getBaseContext(), getString(R.string.receiving),
	...
	private byte[] getLineEncoding(int baudRate) {
		final byte[] lineEncodingRequest = { (byte) 0x80, 0x25, 0x00, 0x00,
				0x00, 0x00, 0x08 };
		switch (baudRate) {
		case 14400:
	...
	private boolean initDevice() {
		...
		if (mUsbConnection == null) {
			...
			Toast.makeText(getBaseContext(),
					getString(R.string.opening_device_failed),
		...
		if (!mUsbConnection.claimInterface(usbInterface, true)) {
			...
			Toast.makeText(getBaseContext(),
					getString(R.string.claimning_interface_failed),
					Toast.LENGTH_LONG).show();
			mUsbConnection.close();
			return false;
		}
		...
		// Set line encoding.
		mUsbConnection.controlTransfer(0x21, 0x20, 0, 0, getLineEncoding(9600),
				7, 0);
		...
		if (mInUsbEndpoint == null) {
			...
			Toast.makeText(getBaseContext(),
					getString(R.string.no_in_endpoint_found), Toast.LENGTH_LONG)
					.show();
			mUsbConnection.close();
			return false;
		}
		...
		if (mOutUsbEndpoint == null) {
			...
			Toast.makeText(getBaseContext(),
					getString(R.string.no_out_endpoint_found),
					Toast.LENGTH_LONG).show();
			mUsbConnection.close();
			return false;
		}
				...
				if (dataToSend == null) {
					...
					String text = String.format(
							getResources().getString(
									R.string.no_extra_in_intent), DATA_EXTRA);
					Toast.makeText(context, text, Toast.LENGTH_LONG).show();
					return;
				}

				mSenderThread.mHandler.obtainMessage(10, dataToSend)
						.sendToTarget();
			} else if (UsbManager.ACTION_USB_DEVICE_DETACHED.equals(action)) {
				Toast.makeText(context, getString(R.string.device_detaches),
						Toast.LENGTH_LONG).show();
		...
		new Thread("arduino_receiver") {
			public void run() {
				...
				while (mUsbDevice != null) {
					...
					final int len = mUsbConnection.bulkTransfer(mInUsbEndpoint,
			...
			mHandler = new Handler() {
				public void handleMessage(Message msg) {
					...
					if (msg.what == 10) {
						...
						final int len = mUsbConnection.bulkTransfer(
								mOutUsbEndpoint, dataToSend, dataToSend.length,
								0);
						if (DEBUG)
							Log.d(TAG, len + " of " + dataToSend.length
									+ " sent.");
						Intent sendIntent = new Intent(
								DATA_SENT_INTERNAL_INTENT);
